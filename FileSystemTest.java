...
class FileSystem {
    ...
    public List<File> findAllHiddenFilesWithSizeLessThen(int size) {
        return files.stream().filter(file -> file.getName().startsWith(".")).filter(file -> file.getSize()<size).sorted().collect(Collectors.toList());
    }

    public int totalSizeOfFilesFromFolders(List<Character> folders) {
        return files.stream().filter(file -> folders.contains(file.folder)).mapToInt(File::getSize).sum();
    }

    public Map<Integer, Set<File>> byYear() {
        return files.stream().collect(Collectors.groupingBy(file -> file.createdAt.getYear(),TreeMap::new, Collectors.toCollection(TreeSet::new)));
    }
